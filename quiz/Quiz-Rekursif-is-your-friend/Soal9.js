...
function virusCheckRecursive(str, viruses) {
  //code here
  if (!viruses || viruses.length === 0 || str.length === 0) {
    return (str.length === 0 || !viruses) ? 'There is no virus' : 0;
  }
  ...
  for (let i = 0; i < virusArray.length; i++) {
    if (str[0].toLowerCase() === virusArray[i].toLowerCase()) {
      count = 1;
      break;
    }
  }

  let result = virusCheckRecursive(str.slice(1), viruses);

  return typeof result === 'string' ? count : count + result;
}
