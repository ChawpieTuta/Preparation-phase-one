...
function travelingIndonesia(arr, emoney) {
  //code here
    ...
    for (let i = 0; i < arr.length; i++) {
        let [name, departureCity, destinationCity, transport] = arr[i].split('-');
        let departureIndex = route.indexOf(departureCity);
        let destinationIndex = route.indexOf(destinationCity);
        let distance = Math.abs(departureIndex - destinationIndex);
        let cost = transportCosts[transport] * distance;
            cost = cost - (cost * discountRates[emoney]);
